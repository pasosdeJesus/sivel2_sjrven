...
CREATE VIEW cben1 AS
 SELECT caso.id AS id_caso,
   ...
   FROM sivel2_gen_caso caso,
    ...
    sip_persona persona
  WHERE ((caso.id = victima.id_caso) AND (caso.id = casosjr.id_caso) AND (caso.id = victima.id_caso) AND (casosjr.oficina_id = 102) AND (persona.id = victima.id_persona));
...
ALTER TABLE ONLY sivel2_gen_victimacolectiva
    ADD CONSTRAINT victimacolectiva_id_grupoper_fkey FOREIGN KEY (id_grupoper) REFERENCES sip_grupoper(id);
